...
Board.prototype.setState = function(stateArray, callback) {
    ...
    stateArray = stateArray.map(s => {
        ...
        if (validCmds.off.indexOf(s) > -1) return "off";
        invalid.push(s);
    });
    if (invalid.length) return callback(new Error(`Invalid state(s) in state array: ${invalid}`));
    ...
    stateArray.forEach((s, i) => {
        this._port.write(commands[i][s], (err, result) => {
            ...
            tried++;
            if (tried == stateArray.length) callback(errors, success);
        });
    });
}

Board.prototype.toggleOne = function(relay, command, callback) {
    if (relay < 1 || relay > 16) return callback(new Error(`Invalid relay: ${relay}`));
...
Board.prototype.toggle = function(toToggle, command, callback) {
    ...
    if (invalid.length) return callback(new Error(`Invalid relays: ${invalid}`));
    ...
    toToggle.forEach(relay => {
        this._port.write(commands[relay - 1][command], (err, result) => {
            ...
            tried++;
            if (tried == toToggle.length) callback(errors, success);
        });
    });
}
