...
var Board = require('./RelayBoard.js');

module.exports = RelayGroup;

function RelayGroup({ test = false, ports = [] } = {}) {
    this.boards = [];
    this.ports = [];
    this.nBoards = 0;
    this.test = test;
...
RelayGroup.prototype.listPorts = function() {
...
RelayGroup.prototype.assignBoards = function(ports) {
    for (let p of ports) {
        p.test = this.test;
...
RelayGroup.prototype.setStates = function(stateArray) {
...
RelayGroup.prototype.toggle = function(toToggle, command) {
    if (!this.nBoards) return new Promise((resolve, reject) => reject(new Error(`No boards have been initialized`)));
...
RelayGroup.prototype.reset = function() {
    if (!this.nBoards) return new Promise((resolve, reject) => reject(new Error(`No boards have been initialized`)));
